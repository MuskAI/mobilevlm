...
VISION_MODEL=/model/JoeyLin/openaiclip-vit-224/clip-vit-large-patch14
...
while true; do
  ...
  if (( memory_free > 70000 )) && (( utilization < 10 )); then
    bash run_chr_23_c-abstractor.sh mobilevlm_v2_1.7b pretrain-finetune ${LANGUAGE_MODEL} ${VISION_MODEL}
    echo "Finished!"
    break
