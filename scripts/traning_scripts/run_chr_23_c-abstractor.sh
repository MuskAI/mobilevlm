...
case ${TASK} in
    "pretrain-finetune-test")
        ...
        LANGUAGE_MODEL=$3
        VISION_MODEL=$4
        bash run_chr_23_c-abstractor.sh ${ARCH} pretrain ${LANGUAGE_MODEL} ${VISION_MODEL} ${OUTPUT_DIR}
        bash run_chr_23_c-abstractor.sh ${ARCH} finetune ${LANGUAGE_MODEL} ${VISION_MODEL} ${OUTPUT_DIR}
        bash run_chr_23_c-abstractor.sh ${ARCH} test ${OUTPUT_DIR}/mobilevlm-2.finetune
    ;;
    "pretrain-finetune")
        cd ${WORK_DIR}
        OUTPUT_DIR=${WORK_DIR}/checkpoint/${ARCH}_$(date +"%Y%m%d_%H%M%S")
        mkdir -p ${OUTPUT_DIR}
        LANGUAGE_MODEL=$3
        VISION_MODEL=$4
        bash run_chr_23_c-abstractor.sh ${ARCH} pretrain ${LANGUAGE_MODEL} ${VISION_MODEL} ${OUTPUT_DIR}
    ...
    "pretrain")
        ...
        VISION_MODEL=$4
        OUTPUT_DIR=$5
        OUTPUT_DIR_PT=${OUTPUT_DIR}/mobilevlm-c-abstractor-23.pretrain
        mkdir -p ${OUTPUT_DIR_PT}
        deepspeed mobilevlm/train/train_mem.py \
            --deepspeed scripts/deepspeed/zero2.json \
            --model_name_or_path ${LANGUAGE_MODEL} \
            --version plain \
            --data_path /data/BJiao/llava/blip_laion_cc_sbu_558k.json \
            --image_folder /data/BJiao/llava/images \
            --vision_tower ${VISION_MODEL} \
            --vision_tower_type clip \
            --mm_projector_type c-abstractor \
    ...
    "finetune")
        ...
        OUTPUT_DIR=$5
        OUTPUT_DIR_PT=${OUTPUT_DIR}/mobilevlm-c-abstractor-23.pretrain
        OUTPUT_DIR_FT=${OUTPUT_DIR}/mobilevlm-c-abstractor-23.finetune
        ...
        deepspeed mobilevlm/train/train_mem.py \
            ...
            --model_name_or_path ${LANGUAGE_MODEL} \
            --version v1 \
            --data_path /data/BJiao/llava/llava1-5/data/llava_v1_5_mix665k_filtered3.json \
            --image_folder /data/BJiao/llava/llava1-5/data/ \
            --vision_tower ${VISION_MODEL} \
            --vision_tower_type clip \
